...
### 目标
- [x] 派新一轮 subagent 继续扫描 server / web / plugin-runtime 真实行为缺陷
- [x] 优先核对阶段 I judge 残余风险与已知未收口项
- [x] 汇总下一批高价值问题并选定首修项
...
### 目标
- [x] `HostModelRoutingPanel` 保存按钮跟真实保存态同步，保存中不可重复触发
- [x] `saveHostModelRoutingConfig()` 旧请求晚回包时不再覆盖新配置
- [x] 聊天页子代理标签轮询旧响应不再覆盖最新标签列表
- [x] 补对应前端回归测试
- [x] 跑完整验证后提交
...
### 目标
- [x] `display result` 生成中也能真正停止，不再把发送队列卡死
- [x] 切会话时若任一详情请求失败，回滚到上一会话状态，不留下半切换空壳
- [x] 补对应聊天 store 回归测试
- [x] 跑完整验证后提交

### 验收补充
- [x] 补齐后端 `display result` stop / 单飞语义，与前端阶段 O 口径一致
- [x] 补后端回归测试并复跑完整验证
