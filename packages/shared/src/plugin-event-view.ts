...
export function buildPluginEventListResult(input: {
  ...
  const items = (hasMore ? input.events.slice(0, input.limit) : input.events).map((event) => ({
    ...
    metadata: parseNullablePluginEventMetadata({
...
function parseNullablePluginEventMetadata(input: {
  raw: string | null;
  onWarn?: (message: string) => void;
}): JsonObject | null {
  if (!input.raw) {
  ...
  try {
    ...
    return isJsonObjectValue(parsed) ? parsed : null;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    input.onWarn?.(`plugin.nullableJsonObject JSON 无效，已回退默认值: ${message}`);
    return null;
  }
}
