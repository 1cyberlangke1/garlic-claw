...
export type InboundHookFamily<
  TChatBeforeModelResult,
  TMessageReceivedResult,
> = {
  'chat:before-model': HookSpec<
    ChatBeforeModelHookPayload,
    TChatBeforeModelResult
  >;
  'message:received': HookSpec<
    MessageReceivedHookPayload,
    TMessageReceivedResult
  >;
};
...
export type MessageHookFamily = {
  'chat:after-model': HookSpec<
    ChatAfterModelHookPayload,
    ChatAfterModelHookPayload
  >;
  'message:created': HookSpec<
    MessageCreatedHookPayload,
    MessageCreatedHookPayload
  >;
  'message:updated': HookSpec<
    MessageUpdatedHookPayload,
    MessageUpdatedHookPayload
  >;
};

export type OperationHookFamily<
  TAutomationBeforeRunResult,
  TToolBeforeCallResult,
> = {
  'automation:before-run': HookSpec<
    AutomationBeforeRunHookPayload,
    TAutomationBeforeRunResult
  >;
  'automation:after-run': HookSpec<
    AutomationAfterRunHookPayload,
    AutomationAfterRunHookPayload
  >;
  'tool:before-call': HookSpec<
    ToolBeforeCallHookPayload,
    TToolBeforeCallResult
  >;
  'tool:after-call': HookSpec<
    ToolAfterCallHookPayload,
    ToolAfterCallHookPayload
  >;
  'response:before-send': HookSpec<
    ResponseBeforeSendHookPayload,
    ResponseBeforeSendHookPayload
  >;
};
...
> = {
  [TName in keyof TFamily]: (
    input: HookChainInput<TFamily[TName][0], TInvokeHook> & {
