import type { ChatMessageMetadata, ChatMessagePart, ConversationContextWindowPreview, PluginCallContext } from '@garlic-claw/shared';
...
export class ConversationMessagePlanningService {
  ...
  async getContextWindowPreview(input: { conversationId: string; modelId?: string; providerId?: string; userId?: string }): Promise<ConversationContextWindowPreview> {
    return this.contextGovernanceService.getContextWindowPreview(input);
  }
  ...
  async createStreamPlan(input: { activePersonaId?: string; abortSignal: AbortSignal; conversationId: string; messageId: string; modelId: string; persona?: ResolvedPersonaPlan; providerId: string; userId?: string }): Promise<ConversationStreamPlan> {
    ...
    const internalBeforeModel = await this.contextGovernanceService.applyBeforeModel({ conversationId: input.conversationId, messages: [...persona.beginDialogs, ...await this.buildModelMessages(input.conversationId, input.messageId)], modelId: input.modelId, providerId: input.providerId, systemPrompt: persona.prompt, userId: input.userId });
    ...
    if (beforeModel.action === 'short-circuit') {return createShortCircuitPlan(beforeModel);}
...
function createShortCircuitPlan(result: Extract<BeforeModelResult, { action: 'short-circuit' }>): ConversationStreamPlan { return { modelId: result.modelId, providerId: result.providerId, responseSource: 'short-circuit', shortCircuitParts: result.assistantParts, stream: createShortCircuitStream(result.assistantContent) }; }
