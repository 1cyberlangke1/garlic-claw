...
export function replaceRuntimeText(content: string, oldString: string, newString: string, replaceAll = false): RuntimeTextReplaceResult {
  ...
  throw new BadRequestException(['edit.oldString 未在文件中找到。', '请重新读取当前文件，确保 oldString 与文件中的文本一致。', '如果目标片段重复出现，请补更多上下文，至少包含前后几行稳定锚点。'].join(' '));
}
...
function replaceAllMatches(content: string, oldString: string, newString: string, strategy: RuntimeTextReplaceStrategy, matches: RuntimeTextMatch[]): RuntimeTextReplaceResult {
  const candidates = Array.from(new Set(matches.map((match) => match.candidate)));
...
function readStrategyMatches(strategy: RuntimeTextReplaceStrategy, content: string, find: string, source: RuntimeTextSource): RuntimeTextMatch[] {
  switch (strategy) {
    ...
    case 'line-ending-normalized': return find.includes('\n') || find.includes('\r') ? readNormalizedMatches(find, source, normalizeLineEndings) : [];
    ...
    case 'indentation-flexible': {
      const lines = readFindLines(find), normalized = normalizeIndentationBlock(lines.join('\n'));
    ...
    case 'context-aware': return readAnchoredMatches(find, source, 'context-aware');
    case 'block-anchor': return readAnchoredMatches(find, source, 'block-anchor');
...
function readExactMatches(content: string, target: string): RuntimeTextMatch[] {
  ...
  while (index <= content.length) {
    ...
    matches.push({ candidate: target, line: lineFromIndex(content, startIndex), startIndex });
...
function readLooseMatches(content: string, find: string, source: RuntimeTextSource, exactCandidate: string, matchesBlock: (block: string) => boolean, skipSingleLineBlock = false): RuntimeTextMatch[] {
  const lines = readFindLines(find), exactMatches = exactCandidate ? readExactMatches(content, exactCandidate) : [];
...
function readNormalizedMatches(find: string, source: RuntimeTextSource, normalize: (line: string) => string, skipIfUnchanged = false): RuntimeTextMatch[] {
  const lines = readFindLines(find), normalized = lines.map(normalize);
...
function readWindowMatches(source: RuntimeTextSource, find: string, findLines: string[], matches: (block: string[], lineIndex: number) => boolean): RuntimeTextMatch[] {
  ...
  for (let lineIndex = 0; lineIndex <= source.lines.length - findLines.length; lineIndex += 1) {
    ...
    if (matches(block, lineIndex)) {result.push({ candidate: restoreTrailingNewline(block.join('\n'), find), line: lineIndex + 1, startIndex: source.starts[lineIndex] ?? 0 });}
  }
  return result;
}

function readAnchoredMatches(find: string, source: RuntimeTextSource, strategy: 'block-anchor' | 'context-aware'): RuntimeTextMatch[] {
  const lines = readFindLines(find);
  if (lines.length < 3) {return [];}
  const first = lines[0].trim(), last = lines[lines.length - 1].trim(), fixedLength = strategy === 'context-aware';
  let best = 0.5;
  const matches: RuntimeTextMatch[] = [];
  for (let start = 0; start < source.lines.length; start += 1) {
    ...
    for (let end = start + (fixedLength ? lines.length - 1 : 2); end < source.lines.length; end += 1) {
      ...
      const score = readAnchorSimilarity(block, lines, strategy);
      if (score < 0.5) {continue;}
      const match = { candidate: restoreTrailingNewline(block.join('\n'), find), line: start + 1, startIndex: source.starts[start] ?? 0 };
...
function readAnchorSimilarity(block: string[], find: string[], strategy: 'block-anchor' | 'context-aware'): number {
  ...
  for (let index = 1; index <= middle; index += 1) {
    ...
    if (strategy === 'context-aware') {
      ...
      total += 1; if (left === right) {matched += 1;} continue;
    }
...
function normalizeReplacement(strategy: RuntimeTextReplaceStrategy, candidate: string, target: string, replacement: string): string {
  const withIndentation = strategy === 'indentation-flexible' ? normalizeIndentationReplacement(candidate, target, replacement) : replacement;
  return normalizeReplacementLineEndings(candidate, withIndentation);
}
...
function readCommonIndentation(value: string): string {
  const lines = trimTrailingEmptyLine(value.split('\n')).filter((line) => line.trim().length > 0);
  return lines.reduce((common, line) => {
    const current = line.match(/^\s*/u)?.[0] ?? '';
    return common.length === 0 || current.length < common.length ? current : common;
  }, lines[0]?.match(/^\s*/u)?.[0] ?? '');
}
...
function lineFromIndex(content: string, startIndex: number): number {
  return startIndex <= 0 ? 1 : content.slice(0, startIndex).split('\n').length;
}

function readFindLines(find: string): string[] {
  return trimTrailingEmptyLine(find.split('\n'));
}
