...
import { renderRuntimeSearchEmptyHint, renderRuntimeSearchReadFollowUpHint, renderRuntimeSearchSuggestedReadHint, renderRuntimeSearchTotalSummary, renderRuntimeSearchTruncationSummary } from '../file/runtime-search-result-report';
...
export interface GlobToolInput { backendKind: RuntimeBackendKind; path?: string; pattern: string; sessionId: string; }
export interface GlobToolResult { count: number; output: string; truncated: boolean; }

const MAX_GLOB_RESULTS = 100;
export const GLOB_TOOL_PARAMETERS: Record<string, PluginParamSchema> = {
  pattern: { description: '要匹配的 glob 模式，例如 `*.ts`、`**/*.md`。', required: true, type: 'string' },
  path: { description: '可选搜索目录。相对路径会基于当前 backend 的可见根解析，默认搜索整个可见文件系统。', required: false, type: 'string' },
};
...
export class GlobToolService {
  ...
  getToolName(): string { return 'glob'; }
  getToolParameters(): Record<string, PluginParamSchema> { return GLOB_TOOL_PARAMETERS; }

  buildToolDescription(): string {
    const { visibleRoot } = this.runtimeSessionEnvironmentService.getDescriptor();
    return ['在当前 backend 可见路径内按 glob 模式列出文件。', visibleRoot === '/' ? 'path 可省略，或传 backend 可见的绝对路径。' : `path 参数只能位于 ${visibleRoot} 内。`, '结果返回 backend 可见路径，不执行任何命令。'].join('\n');
  }

  readInput(args: Record<string, unknown>, sessionId?: string, backendKind?: RuntimeBackendKind): GlobToolInput {
    if (!sessionId) {throw new BadRequestException('glob 工具只能在 session 上下文中使用');}
    const pattern = typeof args.pattern === 'string' ? args.pattern.trim() : '';
    if (!pattern) {throw new BadRequestException('glob.pattern 不能为空');}
    return { backendKind: backendKind ?? this.runtimeFilesystemBackendService.getDefaultBackendKind(), ...(typeof args.path === 'string' && args.path.trim() ? { path: args.path.trim() } : {}), pattern, sessionId };
  }
  ...
  readRuntimeAccess(input: GlobToolInput): RuntimeToolAccessRequest {
    return { backendKind: input.backendKind, metadata: { pattern: input.pattern, ...(input.path ? { path: input.path } : {}) }, requiredOperations: ['file.list'], role: 'filesystem', summary: `按 glob 搜索路径 ${input.path ?? this.runtimeSessionEnvironmentService.getDescriptor().visibleRoot}` };
  }

  toModelOutput: NonNullable<Tool['toModelOutput']> = ({ output }) => ({ type: 'text', value: (output as GlobToolResult).output });
}
