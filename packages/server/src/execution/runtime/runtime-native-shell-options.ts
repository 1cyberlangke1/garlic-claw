...
export interface RuntimeNativeShellOptions {
  defaultTimeoutMs: number;
  descriptor: RuntimeBackendDescriptor;
  maxTimeoutMs: number;
}

export function readRuntimeNativeShellOptions(): RuntimeNativeShellOptions {
  const networkEnabled = readBooleanEnv('GARLIC_CLAW_RUNTIME_NATIVE_SHELL_ENABLE_NETWORK', true);
...
export function readRuntimeNativeShellTimeout(inputTimeout: number | undefined): number {
  const options = readRuntimeNativeShellOptions();
  if (options.defaultTimeoutMs > options.maxTimeoutMs) {
    throw new BadRequestException('native-shell 默认超时不能大于最大超时');
  }
...
function readNetworkPolicy(networkEnabled: boolean): RuntimePermissionPolicy['networkAccess'] {
  ...
  const configuredPolicy = process.env.GARLIC_CLAW_RUNTIME_NATIVE_SHELL_NETWORK_POLICY?.trim();
  if (!configuredPolicy) {
    return 'ask';
  }
  ...
  throw new BadRequestException('GARLIC_CLAW_RUNTIME_NATIVE_SHELL_NETWORK_POLICY 只能是 allow / ask / deny');
}
