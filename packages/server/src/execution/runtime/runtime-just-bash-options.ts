...
export interface RuntimeJustBashOptions {
  defaultTimeoutMs: number;
  descriptor: RuntimeBackendDescriptor;
  maxTimeoutMs: number;
}

export function readRuntimeJustBashOptions(): RuntimeJustBashOptions {
  const networkEnabled = readBooleanEnv('GARLIC_CLAW_RUNTIME_JUST_BASH_ENABLE_NETWORK', true);
...
export function readRuntimeJustBashTimeout(inputTimeout: number | undefined): number {
  const options = readRuntimeJustBashOptions();
  if (options.defaultTimeoutMs > options.maxTimeoutMs) {
    throw new BadRequestException('just-bash 默认超时不能大于最大超时');
  }
...
function readNetworkPolicy(networkEnabled: boolean): RuntimePermissionPolicy['networkAccess'] {
  ...
  const configuredPolicy = process.env.GARLIC_CLAW_RUNTIME_JUST_BASH_NETWORK_POLICY?.trim();
  if (!configuredPolicy) {
    return 'ask';
  }
  ...
  throw new BadRequestException('GARLIC_CLAW_RUNTIME_JUST_BASH_NETWORK_POLICY 只能是 allow / ask / deny');
}
