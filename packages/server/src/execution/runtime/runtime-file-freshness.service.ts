...
interface RuntimeRecentReadEntry extends Pick<RuntimeReadStamp, 'continuationOffset' | 'loadedWindow' | 'readAt'> {
  path: string;
}

interface RuntimeRecentReadOptions {
  excludePath?: string;
  includeOnlyLoadedContext?: boolean;
  limit?: number;
}

interface RuntimeRememberReadOptions {
  lineCount?: number;
  offset?: number;
  totalLines?: number;
  truncated?: boolean;
...
export class RuntimeFileFreshnessService {
  ...
  async assertCanWrite(sessionId: string, filePath: string, backendKind?: RuntimeBackendKind): Promise<void> {
    ...
    if (!stamp) {
      throw new BadRequestException([
        ...
        '请先使用 read 工具读取该文件。',
        ...renderRuntimeRecentReadHints(this.listRecentReadEntries(sessionId, {
          excludePath: stat.virtualPath,
          includeOnlyLoadedContext: true,
          limit: 5,
        })),
      ].join('\n'));
    }
    ...
    throw new BadRequestException([
      ...
      `上次读取: ${stamp.readAt}`,
      ...(stamp.loadedWindow ? [`已过期上下文: ${stamp.loadedWindow}`] : []),
      `请先重新读取当前文件: read ${stat.virtualPath}`,
    ].join('\n'));
  }
  ...
  async rememberRead(
    ...
    options?: RuntimeRememberReadOptions,
  ): Promise<void> {
    const stat = await this.runtimeFilesystemBackendService.statPath(sessionId, filePath, backendKind);
    if (!stat.exists || stat.type !== 'file') {
      return;
    }
    this.readSessionStamps(sessionId).set(stat.virtualPath, {
      ...(options ? readRuntimeReadStampContext(options) : {}),
      mtime: stat.mtime,
  ...
  listRecentReads(sessionId: string, options: RuntimeRecentReadOptions = {}): string[] {
    return this.listRecentReadEntries(sessionId, options).map((entry) => entry.path);
  }

  claimReadInstructionPaths(sessionId: string, paths: string[], assistantMessageId?: string): string[] {
    const scopeKey = assistantMessageId?.trim() ? `${sessionId}::${assistantMessageId.trim()}` : sessionId;
    const claimed = this.readInstructionClaims.get(scopeKey) ?? new Set<string>();
  ...
  listRecentReadEntries(sessionId: string, options: RuntimeRecentReadOptions = {}): RuntimeRecentReadEntry[] {
    ...
    return [...sessionStamps.entries()]
      .filter(([virtualPath, stamp]) => (
        (!excludePath || virtualPath !== excludePath)
        && (!(options.includeOnlyLoadedContext ?? false) || Boolean(stamp.loadedWindow || stamp.continuationOffset))
      ))
  ...
  buildReadSystemReminder(sessionId: string, options: RuntimeRecentReadOptions = {}): string[] {
    ...
    return recentReads.length === 0
      ...
      : [
        ...
        ...renderRuntimeRecentReadEntries(recentReads),
  ...
  async withFileLock<T>(
    ...
    const previous = this.fileLocks.get(lockKey) ?? Promise.resolve();
    let release!: () => void;
    ...
    try {
      ...
      release();
      if (this.fileLocks.get(lockKey) === queued) {
  ...
  private readSessionStamps(sessionId: string): Map<string, RuntimeReadStamp> {
    ...
    if (!sessionStamps) {
      sessionStamps = new Map<string, RuntimeReadStamp>();
...
function renderRuntimeRecentReadHints(recentReads: RuntimeRecentReadEntry[]): string[] {
  return recentReads.length === 0
    ? []
    : ['本 session 已加载的其他文件上下文（按最近读取排序）：', ...renderRuntimeRecentReadEntries(recentReads)];
}

function renderRuntimeRecentReadEntries(recentReads: RuntimeRecentReadEntry[]): string[] {
  return recentReads.map((entry, index) => {
    const age = readRuntimeRecentReadAge(entry.readAt);
    const header = age ? `[最近读取 #${index + 1} | ${age}]` : `[最近读取 #${index + 1}]`;
    const detail = [
      entry.loadedWindow,
      entry.continuationOffset ? `下一步可 read ${entry.path} offset=${entry.continuationOffset}` : '当前窗口已加载，可直接复用',
...
function readRuntimeReadStampContext(options: RuntimeRememberReadOptions): Pick<RuntimeReadStamp, 'continuationOffset' | 'loadedWindow'> {
  ...
  if (lineCount <= 0) {
    return options.totalLines === 0 ? { loadedWindow: '已加载空文件（0 lines of 0）' } : {};
  }
  const end = offset + lineCount - 1;
  return {
    continuationOffset: options.truncated ? end + 1 : undefined,
    loadedWindow: options.totalLines
      ? `已加载 lines ${offset}-${end} of ${options.totalLines}`
      : `已加载 lines ${offset}-${end}`,
  };
}
