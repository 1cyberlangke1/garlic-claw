...
export function createPluginConfigSnapshot(record: RegisteredPluginRecord): PluginConfigSnapshot {
  return { schema: record.manifest.config ?? null, values: resolvePluginConfigValues(record) };
}
...
export function buildPluginSelfSummary(record: RegisteredPluginRecord): JsonObject {
  ...
  return {
    ...
    runtimeKind: record.manifest.runtime,
    version: record.manifest.version,
    ...collectPluginSelfCapabilities(record),
  };
}
...
export function buildPluginCommandOverview(commands: PluginCommandInfo[]): PluginCommandOverview {
  ...
  const conflicts = buildPluginCommandConflicts(sortedCommands)
    .map((conflict) => ({ ...conflict, commands: [...conflict.commands].sort(comparePluginCommandConflictEntriesForCatalog) }))
...
function collectPluginSelfCapabilities(record: RegisteredPluginRecord): JsonObject {
  return Object.fromEntries(PLUGIN_SELF_CAPABILITY_KEYS.flatMap((key) => record.manifest[key]?.length ? [[key, record.manifest[key]]] : [])) as unknown as JsonObject;
}
...
function resolvePluginConfigValues(record: RegisteredPluginRecord): JsonObject {
  return resolveConfigNodeValue(record.manifest.config ?? null, record.configValues ?? {}) as JsonObject;
}
...
function comparePluginCommandsForCatalog(left: PluginCommandInfo, right: PluginCommandInfo): number {
  return comparePluginCommandIdentity(left, right);
}
...
function comparePluginCommandConflictEntriesForCatalog(left: PluginCommandConflictEntry, right: PluginCommandConflictEntry): number {
  return comparePluginCommandIdentity(left, right);
}

function comparePluginCommandIdentity(
  left: Pick<PluginCommandInfo, 'canonicalCommand' | 'commandId' | 'kind' | 'pluginId' | 'priority'>,
  right: Pick<PluginCommandInfo, 'canonicalCommand' | 'commandId' | 'kind' | 'pluginId' | 'priority'>,
): number {
