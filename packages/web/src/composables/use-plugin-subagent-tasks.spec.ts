...
import { usePluginSubagentTasks } from './use-plugin-subagent-tasks'
import * as api from '../api'

vi.mock('../api', () => ({
  listPluginSubagentTaskOverview: vi.fn(),
}))
...
describe('usePluginSubagentTasks', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.clearAllMocks()
    vi.mocked(api.listPluginSubagentTaskOverview).mockResolvedValue(createOverview())
  })
  ...
  it('loads the overview, filters by keyword and task status, and keeps polling', async () => {
    ...
    expect(api.listPluginSubagentTaskOverview).toHaveBeenCalledTimes(2)
