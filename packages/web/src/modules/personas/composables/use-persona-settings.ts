...
export interface PersonaPresetDraftInput {
  beginDialogs?: PluginPersonaDialogEntry[]
  customErrorMessage?: string
  description?: string
  id?: string
...
interface PersonaBatchApplyResult {
  appliedConversationCount: number
...
export function usePersonaSettings() {
  ...
  const batchConversationIds = ref<string[]>([])
  const batchApplyingPersona = ref(false)
  const batchApplyResult = ref<PersonaBatchApplyResult | null>(null)
  const autoGeneratedId = computed(() => generatePersonaId(editorDraft.value.name))

  const currentConversationId = computed<string | null>(() => unref(chat.currentConversationId) ?? null)
  const availableConversations = computed<Conversation[]>(() => (
    (unref(chat.conversations) ?? []).filter((conversation) => conversation.kind !== 'subagent')
  ))
  ...
  const canApplySelectedPersonaToBatch = computed(() => (
    Boolean(selectedPersonaId.value) && batchConversationIds.value.length > 0
  ))
  ...
  watch(
    ...
    (newId, oldId) => {
      ...
      if (editorDraft.value.id === '' || editorDraft.value.id === oldId) {
        editorDraft.value.id = newId
      }
    },
  )
  ...
  watch(
    ...
    (conversations) => {
      ...
      batchConversationIds.value = batchConversationIds.value.filter((conversationId) => validConversationIds.has(conversationId))
    },
    { deep: true },
  )
  ...
  function loadPresetDraft(preset: PersonaPresetDraftInput) {
    editorMode.value = 'create'
    selectedPersonaId.value = null
    selectedPersona.value = null
    deleteResult.value = null
    ...
    editorDraft.value = {
      ...createEmptyDraft(),
      beginDialogs: (preset.beginDialogs ?? []).map((entry) => ({ ...entry })),
      customErrorMessage: preset.customErrorMessage ?? '',
      description: preset.description ?? '',
      id: preset.id ?? '',
      isDefault: preset.isDefault ?? false,
      name: preset.name,
      prompt: preset.prompt,
      toolInput: preset.toolInput ?? '',
      toolMode: preset.toolMode ?? 'all',
    }
  }
  ...
  function toggleBatchConversation(conversationId: string) {
    if (batchConversationIds.value.includes(conversationId)) {
      batchConversationIds.value = batchConversationIds.value.filter((id) => id !== conversationId)
      return
    }
    batchConversationIds.value = [...batchConversationIds.value, conversationId]
  }

  function selectAllBatchConversations() {
    batchConversationIds.value = availableConversations.value.map((conversation) => conversation.id)
  }

  function clearBatchConversations() {
    batchConversationIds.value = []
  }

  async function applySelectedPersonaToBatch() {
    if (!selectedPersonaId.value || batchConversationIds.value.length === 0) {
      return
    }

    const personaId = selectedPersonaId.value
    const targetConversationIds = [...new Set(batchConversationIds.value)]
    const failedConversationIds: string[] = []
    batchApplyingPersona.value = true
    batchApplyResult.value = null
    requestState.clearError()
    try {
      for (const conversationId of targetConversationIds) {
        try {
          await activateConversationPersona(conversationId, personaId)
        } catch {
          failedConversationIds.push(conversationId)
        }
      }

      if (currentConversationId.value && targetConversationIds.includes(currentConversationId.value)) {
        await refreshCurrentPersona()
      }

      batchApplyResult.value = {
        appliedConversationCount: targetConversationIds.length - failedConversationIds.length,
        failedConversationIds,
        personaName: selectedPersona.value?.name ?? personaId,
      }

      if (failedConversationIds.length > 0) {
        throw new Error(`以下会话应用失败：${failedConversationIds.join('、')}`)
      }
    } catch (caughtError) {
      requestState.setError(caughtError, '批量应用 Persona 失败')
    } finally {
      batchApplyingPersona.value = false
    }
  }
