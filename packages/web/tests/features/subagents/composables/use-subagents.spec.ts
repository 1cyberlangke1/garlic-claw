...
describe('useSubagents', () => {
  ...
  it('loads overview, groups workspaces and keeps subagent index state only', async () => {
    ...
    expect(state.conversationWorkspaces.value[0]?.subagents.map((subagent) => subagent.conversationId)).toEqual([
